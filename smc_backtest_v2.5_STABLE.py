...
def fetch_twelvedata(api_key: str, symbol: str, interval: str, start: Optional[str], end: Optional[str]) -> pd.DataFrame:
    ...
    print(f"[FETCH] Fetch {symbol} {interval} {start} -> {end} ...")
...
def backtest_smc(
    ...
    def try_short_entry(i: int, rr_use: float, use_ct=False) -> bool:
        ...
        # ALLOCATION ADAPTATIVE basée sur la qualité du signal + Kelly
        ...
        risk_factor = base_risk_factor * quality_multiplier * kelly_multiplier
...
def main():
    ...
    print(f"[DATE] Période: {args.start} -> {args.end}")
    ...
    print(f"[SAVED] LTF saved: {ltf_csv}")
    ...
    print(f"[SAVED] HTF saved: {htf_csv}")
    ...
    if trades:
        ...
        print("[CHART]  equity_curve.png sauvegardé.")
        plt.show()
    else:
